    ...
    fetch('/api/products')
      ...
      .then((result) => {
        if (result.success && result.data) {
          ...
          const product = result.data.find((p: any) => p.id.toString() === productId);
          if (product) {
            setForm({
              name: product.name || '',
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl">
      <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/20 p-6 md:p-8 shadow-sm flex flex-col gap-6">
        {error && (
          <div className="bg-error-container/30 text-error text-sm px-4 py-3 rounded-xl border border-error/20">{error}</div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="number" min="0" value={form.promo_price} onChange={(e) => setForm((p) => ({ ...p, promo_price: e.target.value }))}
