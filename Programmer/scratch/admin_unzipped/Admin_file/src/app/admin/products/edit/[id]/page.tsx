    ...
    fetch('/api/products')
      ...
      .then((result) => {
        if (result.success && result.data) {
          ...
          const product = result.data.find((p: { id: number; name: string; category_id: number; description: string; original_price: number; stock_quantity: number; image_url: string }) => p.id.toString() === productId);
          if (product) {
            setForm({
              name: product.name || '',
